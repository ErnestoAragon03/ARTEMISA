...
silence_threshold = 3    #Segundos de silencio antes de detener la grabación
...
#Captura el audio
def capture_audio():
    logger.info("Llamada a capture_audio")
    try:
        global last_voice_time
        ...
        temp_files_dir = os.path.join(base_path, 'ASR', 'Online', 'Temp')
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False, dir=temp_files_dir) as temp_audio_file:
        ...
        logger.error("Error en capture_audio: %s", e)
...
def transcribe():
    ...
    try:
        ...
        logger.error("Error en cloud_asr.py: %s", e)
        print("Error: ", e)